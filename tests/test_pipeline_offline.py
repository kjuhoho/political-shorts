...
def test_violated_invariants_are_fed_back_to_the_writer_and_hold_the_video(tmp_path, monkeypatch):
    """The mechanical checks sit next to the AI score: a script that voices one party only gets told so on
    the next attempt, and if it is still one-sided at the end the video is held, however nice its score."""
    ...
    monkeypatch.setattr(quality_agent, "review", lambda script, cfg: quality_agent.AgentReport(
        available=True, score=96, issues=[]))                      # the AI judge is happy ...
    seen = []
    ...
    assert 'script.get("invariant_violations")' in src and "hold_publish = (not qr.publishable) or bool(_viol)" in src
