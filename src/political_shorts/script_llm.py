...
_LLM_LIMIT = {"summary": 220, "what": 260, "reaction": 130,
              "factcheck": 150, "sides": 340, "outro": 170}
...
_SYSTEM = (
    ...
    "2) 카드가 이야기처럼 이어지게: 이 영상 전체가 뉴스 리포트 한 편처럼 처음부터 "
    "끝까지 하나로 이어져야 합니다. 각 카드를 그 카드만 떼어놓고 봐도 말이 되는 "
    "독립된 사실 나열로 쓰지 말 것 — 반드시 앞 카드에서 나온 내용을 다음 카드 "
    "첫머리에서 다시 받아 이어갈 것. '그런데', '문제는 이겁니다', '쉽게 말하면', "
    "'이런 상황 속에서'처럼 내용 있는 문장 안에 붙는 연결어를 쓸 것. 특히 what -> "
    "sides로 넘어갈 때는 '무슨 일이 있었다' 다음 곧바로 '이런 상황 속에서 여당은 "
    "…, 야당은 …'처럼 방금 나온 사건을 두고 각 진영이 실제로 뭐라 반응했는지로 "
    "이어질 것 — sides를 what과 무관한 별개 화제로 던지지 말 것.\n"
    ...
    "그 이유(why)까지 살려 쓸 것 — 그냥 입장만 나열하지 말 것. 정당 입장도 원문에 "
    "실제로 근거가 있는 쪽만 넣을 것 — 한쪽 정당의 입장만 확인되면 그 한쪽만 쓰고, "
    "반대쪽 입장을 지어내거나 '다른 목소리도 있다'는 식으로 얼버무리지 말 것. "
    "[분석 1단계]의 sides에 교수·연구위원 등 제3의 전문가 평가가 있으면 (있는 만큼의) "
    "정당 입장에 이어 반드시 추가 목소리로 넣을 것 — 실제 뉴스 리포트처럼 '이런 상황 "
    ...
    "무엇을 지켜봐야 하는지까지 구체적으로. 그 다음 마지막 문장은 반드시 이 사건에 대해 "
    "시청자의 의견을 묻는 구체적이고 중립적인 질문 한 개 + '댓글로 남겨주세요'로 끝낼 것 "
    "(예: '김승원 후보자의 사퇴, 여러분은 어떻게 보시나요? 댓글로 의견을 남겨주세요.'). "
    "질문은 어느 한쪽 답을 유도하지 말 것. '구독과 좋아요 눌러주세요'만 덩그러니 붙이는 "
    "것은 금지.\n\n"
    ...
    "title(화면 상단 2줄 제목이자 유튜브 실제 제목): 실제 올라간 인기 영상 제목 "
    "형식을 따를 것 — 썸네일에 큰 글씨로 들어가므로 전체 3~5단어로 아주 짧게. 1줄은 "
    "인물/사건(2~3단어, 12자 이내, 예: '한동훈 녹취록 공개', '김승원 전격 사퇴'), 2줄은 "
    "궁금증을 자극하는 짧은 질문(1~3단어, 10자 이내, 예: '진짜 이유는?', '왜 논란인가', "
    "'무슨 일일까?'). 긴 부제·수식어 금지. '지금 이 이슈', "
    "'핵심만', '쟁점 정리' 같은 맹탕 문구 절대 금지. 내용과 반드시 일치, 과장·비하 "
    "없이. 대립하는 두 주체(인물·기관·정당)가 뚜렷하면 1줄을 'A vs B' 형태로 압축할 "
    "것 — 실제로 가장 반응이 좋았던 제목: '대통령 vs 대법원장 정면충돌?'. 이 title은 "
...
_VAGUE_OUTRO_END = re.compile(
    ...
    r"살펴볼\s*필요가\s*있습니다|지켜볼\s*필요가\s*있습니다|눈여겨볼\s*필요가\s*있습니다|"
    r"확인이\s*필요합니다)\s*\.?\s*$"
)
