...
_SYSTEM = (
    ...
    "그 이유(why)까지 살려 쓸 것 — 그냥 입장만 나열하지 말 것. [분석 1단계]의 sides에 "
    "교수·연구위원 등 제3의 전문가 평가가 있으면 정당 둘의 입장에 이어 반드시 세 "
    "번째 목소리로 넣을 것 — 실제 뉴스 리포트처럼 '이런 상황 속에서 여당은 …, "
    "야당은 …, 그리고 OO 교수는 …라고 짚었습니다' 순서로.\n"
    ...
    "'왜 그렇게 보는지' 근거를 붙여 2~3문장. [분석 1단계]의 sides에 교수·연구위원 "
    "등 제3의 전문가 평가가 실제로 있으면, 정당 두 입장을 소개한 다음 반드시 세 "
    "번째 목소리로 이어 붙일 것 — 실제 뉴스 리포트가 여러 입장을 순서대로 소개하듯 "
    ...
    "형식을 따를 것 — 1줄은 유명 인물/사건을 구체적으로(예: '한동훈 녹취록 공개', "
    "'이재명 대통령 임기 발언'), 2줄은 구어체 궁금증 종결(예: '유출 경위 조사할까?', "
    "'무슨 일일까요?', '진짜일까?', '왜 논란인가'). 각 줄 16자 이내. '지금 이 이슈', "
